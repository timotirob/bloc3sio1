...
. Vous pouvez dézipper le programme fourni `curl-8.18.0.zip` dans le répertoire `c:\Program Files`.
+
Pour se faire:

* copier le fichier Zip dans ce répertoire
* ouvrir une commande PowerShell en administrateur
* cd "c:\Program Files"
* tar -xf curl-8.18.0.zip
+
. Ensuite vous pouvez ajouter le répertoire `bin` à la variable d’environnement `PATH`.
+
Exemple de chemin à ajouter :
`C:\Program Files\curl-8.18.0\bin`
